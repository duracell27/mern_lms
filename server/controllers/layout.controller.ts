...
export const createLayout = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isTypeExist) {
        return next(
          new ErrorHandler("Layout with this type already exist", 400)
        );
      }
      if (type === "Banner") {
        const { image, title, sutTitle } = req.body;
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (type === "Banner") {
          ...
          await cloudinary.v2.uploader.destroy(bannerData.image.publick_id)
